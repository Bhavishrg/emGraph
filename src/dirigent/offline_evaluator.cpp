    ...
    for (const auto& gate : level) {
      switch (gate->type) {
          ...
          for (const auto& multk_level : multk_circ.gates_by_level) {
            for (auto& multk_gate : multk_level) {
              switch (multk_gate->type) {
                ...
                case quadsquad::utils::GateType::kMul4:{
                  const auto* g = static_cast<quadsquad::utils::FIn4Gate*>(multk_gate.get());
        ...
        case quadsquad::utils::GateType::kLtz: {
          ...
          const auto* ltz_g = static_cast<quadsquad::utils::FIn1Gate*>(gate.get());
          // padded_val
          Field padded_val;
          rgen_.all().random_data(&padded_val, sizeof(Field));
          ...
          for (const auto& prefixAND_level : prefixAND_circ.gates_by_level) {
            for (auto& prefixAND_gate : prefixAND_level) {
              switch (prefixAND_gate->type) {
                ...
                case quadsquad::utils::GateType::kAdd: {
                  const auto* g = static_cast<quadsquad::utils::FIn2Gate*>(prefixAND_gate.get());
                ...
                case quadsquad::utils::GateType::kConstAdd: {
                  const auto* g = static_cast<quadsquad::utils::ConstOpGate<BoolRing>*>(prefixAND_gate.get());
                ...
                case quadsquad::utils::GateType::kMul3: {
                  const auto* g = static_cast<quadsquad::utils::FIn3Gate*>(prefixAND_gate.get());
                ...
                case quadsquad::utils::GateType::kMul4:{
                  const auto* g = static_cast<quadsquad::utils::FIn4Gate*>(prefixAND_gate.get());
                ...
                case quadsquad::utils::GateType::kDotprod: {
                  const auto* g = static_cast<quadsquad::utils::SIMDGate*>(prefixAND_gate.get());
