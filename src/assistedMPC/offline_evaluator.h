...
namespace assistedMPC {
    class OfflineEvaluator {
        ...
        static void randomShare_Helper(int nP, RandGenPool& rgen,
                AuthAddShare<Field>& share, TPShare<Field>& tpShare, Field key, 
                std::vector<Field> keySh, std::vector<std::vector<Field>>& rand_sh);

        static void randomShare_Party(AuthAddShare<Field>& share, Field key, 
                std::vector<Field>& rand_sh, size_t& idx_rand_sh);
        ...
        static void randomShareSecret_Helper(int nP, RandGenPool& rgen, 
                AuthAddShare<Field>& share, TPShare<Field>& tpShare, Field secret,
                Field key, std::vector<Field> keySh, std::vector<std::vector<Field>>& rand_sh_sec);

        static void randomShareSecret_Party(AuthAddShare<Field>& share, Field key, std::vector<Field>& rand_sh_sec, size_t& idx_rand_sh_sec);


        static void randomShareWithParty_Helper(int nP, int dealer, RandGenPool& rgen, AuthAddShare<Field>& share, TPShare<Field>& tpShare, Field key, std::vector<Field> keySh, std::vector<std::vector<Field>>& rand_sh_party); 


        static void randomShareWithParty_Party(AuthAddShare<Field>& share, Field key, std::vector<Field>& rand_sh_party, size_t& idx_rand_sh_party); 

        static void randomShareWithParty_Dealer(Field& secret, AuthAddShare<Field>& share,  Field key, std::vector<Field>& rand_sh_party, size_t& idx_rand_sh_party); 
        ...
        // subprotocols.
        void setWireMasksHelper(const std::unordered_map<common::utils::wire_t, int>& input_pid_map, 
