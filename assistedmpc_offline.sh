...
dir=~/benchmark_data/assistedmpc_offline
highestPlayer=$3
startPlayer=4
increment=4
...
then
    ...
    do
        ...
        do
            ...
            if test $party = 1
            then
                ./benchmarks/assistedmpc_offline -p $party --localhost -g $1 -d $2 -n $players -o $json 2>&1 | cat >> $log &
            else
                ./benchmarks/assistedmpc_offline -p $party --localhost -g $1 -d $2 -n $players 2>&1 | cat > /dev/null &
            fi
            codes[$party]=$!
        done

        ./benchmarks/assistedmpc_offline -p 0 --localhost -g $1 -d $2 -n $players -o $dir/g_$1_d_$2_0.json 2>&1 | tee -a $dir/g_$1_d_$2_0.log &
        codes[0]=$!
