  ...
  BOOST_AUTO_TEST_CASE(random_share_with_party) {
    ...
    parties.push_back(std::async(std::launch::async, [&, i]() { 
        ...
        OfflineEvaluator::randomShareWithParty(nP, i, nP, vrgen, *network, 
                                               share, tpshares, secret, key, keySh, rand_sh_party, idx_party);
        return share;
      }));
    ...
    for ( i = 1; i <= nP; i++) {
      ...
      parties.push_back(std::async(std::launch::async, [&, i]() { 
        ...
        OfflineEvaluator::randomShareWithParty(nP, i, nP, vrgen, *network, 
                                               share, tmp, secret, key, keySh, rand_sh_party);
  ...
  BOOST_AUTO_TEST_CASE(random_share_secret) {
    ...
    parties.push_back(std::async(std::launch::async, [&, i]() { 
        ...
        OfflineEvaluator::randomShareSecret(nP, i, vrgen, *network, 
                                               share, tpshares, secret, key, keySh, rand_sh_sec);
    ...
    for ( i = 1; i <= nP; i++) {
      ...
      parties.push_back(std::async(std::launch::async, [&, i]() { 
        ...
        OfflineEvaluator::randomShareSecret(nP, i, vrgen, *network, 
                                               share, tmp, secret, key, keySh, rand_sh_sec);
...
BOOST_AUTO_TEST_CASE(EQZ) {
  ...
  auto w_eq = circ.addGate(quadsquad::utils::GateType::kEqz, input_wires[0]);
  auto w_aab =
...
BOOST_AUTO_TEST_CASE(LTZ) {
  ...
  auto w_lt = circ.addGate(quadsquad::utils::GateType::kLtz, input_wires[0]);
  auto w_aab =
      circ.addGate(quadsquad::utils::GateType::kAdd, input_wires[0], input_wires[1]);
  auto w_cmd =
      circ.addGate(quadsquad::utils::GateType::kMul, input_wires[2], input_wires[3]);
...
// BOOST_AUTO_TEST_CASE(Mult3_bool) {
//   int nP = 6;
//   quadsquad::utils::Circuit<BoolRing> circ;
//   std::vector<quadsquad::utils::wire_t> input_wires;
//   std::unordered_map<quadsquad::utils::wire_t, int> input_pid_map;
