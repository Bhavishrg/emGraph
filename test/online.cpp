...
BOOST_AUTO_TEST_CASE(EQZ_zero) {
  ...
  auto w_eqz =
     circ.addGate(quadsquad::utils::GateType::kEqz, input_wires[0]);
...
BOOST_AUTO_TEST_CASE(Multk_bool) {
  ...
  std::unordered_map<quadsquad::utils::wire_t, int> input_pid_map(64);
  std::unordered_map<quadsquad::utils::wire_t, BoolRing> input_map(64);
  std::unordered_map<quadsquad::utils::wire_t, BoolRing> bit_mask_map(64);
